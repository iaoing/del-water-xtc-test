...
int Pxtc::read_xtc(const string expanded, char *buf, size_t size, off_t offset)
{
    ...
    map< string, xtc_read_buf >::iterator rb_iter;
    ...
    if(xtc_ret < 0)
    {
        ...
        read_xtc_frame(expanded, &(rb_iter->second));
...
int Pxtc::read_add_xi(const string expanded)
{
    int ret = 0;
    ...
    offset = 0;
    size = TAG_SIZE;
    ret = plfs_read(expanded, tag_buf, size, offset, &bytes_read);
...
int Pxtc::read_xtc_first_frame(const string expanded, xtc_read_buf *xrbuf)
{
    // cout << "--> read_xtc_first_frame! " << endl;
    int ret = 0;
    size_t size, bytes_read;
    off_t offset;
    char *buf;
    ...
    ret = plfs_read(expanded, buf, size, offset, &bytes_read);
    if(ret != PLFS_SUCCESS || bytes_read != size)
    {
        free(buf);
        return -99;
    ...
    // check tag magic number, xtc magic number, natoms and bytecnt;
    int tag_magic = 0, xtc_magic = 0, bytecnt = 0, natoms;
    ...
    get_int(buf + offset, &xtc_magic);
    get_int(buf + offset + 4, &natoms);
    get_bcnt_from_buf(buf + offset, &bytecnt);
    if(xtc_magic != 1995 || natoms < 0 || natoms > 99999 
        || bytecnt < 0 || bytecnt > 3 * 4 * natoms)
    {
        xrbuf->trunc_flag = -3;
        return -1;
    }
    ...
    xrbuf->offset = bytes_read;
    xrbuf->f_offset = offset + bytes_read;
    xrbuf->size = bytes_read;
    xrbuf->cf_bytes = bytecnt;
    xrbuf->frame_cnt = 1;
    return 0;
...
int Pxtc::read_xtc_frame(const string expanded, xtc_read_buf *xrbuf)
{
    int ret = 0;
    size_t size, bytes_read;
    off_t offset;
    ...
    int xtc_magic, b_byte, e_byte, bytecnt, natoms;
    memcpy((char*)(&b_byte), buf, sizeof(int));
    memcpy((char*)(&e_byte), buf + sizeof(int), sizeof(int));
    get_int(buf + 2 * sizeof(int), &xtc_magic);
    get_int(buf + 2 * sizeof(int) + 4, &natoms);
    get_bcnt_from_buf(buf + 2 * sizeof(int), &bytecnt);
    free(buf);
    if(xtc_magic != 1995 || natoms < 0 || natoms > 99999 || bytecnt > 3 * 4 * natoms
        || b_byte < 0 || e_byte < 0 || (b_byte + e_byte > bytecnt))
    {
        xrbuf->trunc_flag = -3;
        return -1;
    }
    xrbuf->f_offset += 2 * sizeof(int);        // skip b_byte and e_byte;
    ...
    xrbuf->buffer.resize(xrbuf->size + HEADER_SIZE + bytecnt + 1);

    ret = plfs_read(expanded, (char*)(&(xrbuf->buffer[xrbuf->size])), size, offset, &bytes_read);
    if(ret != PLFS_SUCCESS || bytes_read != size)
    {
        return -99;
    }
    xrbuf->f_offset += bytes_read;
    xrbuf->size += bytes_read;

    // reading the next part of the coordinate;
    offset = xrbuf->f_offset;
    size = e_byte;
    ret = plfs_read(expanded, (char*)(&(xrbuf->buffer[xrbuf->size + water_fill_size])), size, offset, &bytes_read);
    if(ret != PLFS_SUCCESS || bytes_read != size)
    {
        return -99;
    }
    xrbuf->f_offset += bytes_read;
    xrbuf->size += water_fill_size + bytes_read;
    xrbuf->offset += HEADER_SIZE + bytecnt;
...
int Pxtc::read_xtc_check(off_t offset, size_t size, const xtc_read_buf *xrbuf)
{
    printf("begin read_xtc_check\n\t%10d%10d%10d%10d%10d\n", offset, size, xrbuf->offset, xrbuf->size, xrbuf->f_offset);
    // if true means the buffer does meet the data requested;
    if((offset >= xrbuf->offset - xrbuf->size)
        && (offset + size <= xrbuf->offset))
    {
        return 0;
    }
...
int Pxtc::read_xtc_cp_buf(xtc_read_buf *xrbuf, char *buf, size_t size, off_t offset)
{
    ...
    cp_offset = offset - (xrbuf->offset - xrbuf->size);
    cp_size = size;
...
int Pxtc::read_xtc_resize_buf(xtc_read_buf *xrbuf)
{
    if(xrbuf->size > xrbuf->cf_bytes * 5)
    {
        xrbuf->buffer.erase(0, xrbuf->cf_bytes);
        xrbuf->size -= xrbuf->cf_bytes;
    }
...
int Pxtc::xtc_write_close(string strPath)
{
    ...
    // if(hb_iter->second.trunc_flag == 1)
    // {
    //     ret = xtc_trunc_flush_frame(strPath, fd, 250);
    ...
    // {
    //     // means not need trunc and some data not flushed.
    //     ret = xtc_flush_buffer(strPath, fd, 250);
    // }
...
int Pxtc::read_file(const char *path, char *buf, size_t size, off_t offset)
{
    int ret = -2;
    ret = read_xtc(path, buf, size, offset);
    if(ret == -2)
    {
        cout << "---->> plfs_read" << endl;
    }else if(ret == 0)
