...
int main(int argc, char const *argv[])
{
    ...
    string hold_buf;
    hold_buf.resize(buf_size);
    memcpy((char*)(&(hold_buf[0])), buf, buf_size);

    cout << "hold_buf.size: " << hold_buf.size() << endl;
    xtc_get_water_no(&hold_buf, maxWno, 2, wn_index);

    cout << "wn_index: " << wn_index[0] << "  " << wn_index[1] << endl;
